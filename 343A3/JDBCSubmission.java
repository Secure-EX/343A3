...
public abstract class JDBCSubmission {
  ...
  public static class ElectionResult implements Serializable {
    ...
    public boolean equals(Object obj) {
      ...
      return this.presidents.equals(other.presidents) && this.parties.equals(other.parties);
    }
   ...
   *
   * The Jaccard similarity is a measure used for comparing the similarity of sets. The Jaccard
   * similarity of two strings is defined on the set of tokens of the strings. It is the size of the
   * intersection divided by the size of the union of the token sets.
   *
  ...
  protected double similarity(String a, String b) {
    ...
    final long sum =
        (sa + 1 > 0 ? sa : left.stream().count()) + (sb + 1 > 0 ? sb : right.stream().count());
   ...
   *
   * Establishes a connection to be used for this session, assigning it to the instance variable
   * 'connection'. In addition, sets the search path to parlgov.
   *
  ...
  /**
   * Returns the list of presidents over the years in descending order of years and the parties they
   * belong to.
   *
   * @param countryName name of the country
   * @return a 2-D array with the first row being the list of presidents over the years in
   *         descending order of years and the second row being the name of the parties they belong
   *         to.
   */
   ...
   *
   * Does so by finding parties that have similar descriptions using Jaccard similarity and a
   * threshold.
   *
   * @param partyId id of the party
   * @param threshold Jaccard similarity threshold
   * @return a list of parties with Jaccard similarity of descriptions above the given threshold
   */
